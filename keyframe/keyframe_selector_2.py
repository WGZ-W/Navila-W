...
# 不再导入 YOLODetector
from clip_wrapper import SigLIPModelWrapper
...
class FrameData:
    ...
    detected_objects: List[Dict]  # 保留字段但始终为空列表
...
class KeyframeSelector:
    """
    图像关键帧选择器 - 直接处理图像列表
    修改后：不区分名词词性，不使用物体检测，所有名词与整图相似度匹配
    """
    ...
    def __init__(
            ...
            detector_model_type: str = "yolov8",   # 保留参数但不再使用
            detector_model_path: Optional[str] = None,  # 保留参数但不再使用
        ...
        参数:
            ...
            detector_model_type: 已废弃，不再使用
            detector_model_path: 已废弃，不再使用
        ...
        logger.info("注意：已禁用名词类型区分和物体检测，所有名词均与整图进行相似度计算")
    ...
    def extract_nouns_from_text(self, instruction_text: str) -> List[NounInfo]:
        ...
        # 创建NounInfo对象，统一将type设为"scene"（忽略原始类型）
        nouns = []
    ...
    def process_single_image(self, image: Image.Image, frame_idx: int) -> FrameData:
        ...
        # 不再进行物体检测，detected_objects 置为空列表
        return FrameData(
            frame_idx=frame_idx,
            image=image,
    ...
    def select_keyframes_for_nouns(self, nouns: List[NounInfo], similarity_matrix: np.ndarray,
        ...
        for k, noun in enumerate(nouns):
            ...
            if max_score >= self.threshold:
                ...
                logger.info(f"名词 '{noun.text}': 选择图像 {max_index}, 得分 {max_score:.3f}")
            else:
                logger.warning(f"名词 '{noun.text}': 得分 {max_score:.3f} 低于阈值 {self.threshold}, 丢弃")
...
if __name__ == "__main__":
    ...
    print("\n测试预提取特征版本")
    feature_dim = 512
    image_features = [torch.randn(feature_dim) for _ in range(10)]
    feature_result = selector.process_images_with_features(image_features, instruction_text)
