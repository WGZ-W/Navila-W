...
from noun_extractor import EnglishNounExtractor
...
class KeyframeSelector:
    ...
    def process_single_image(self, image: np.ndarray, frame_idx: int) -> FrameData:
        """
        处理单张图像：
        1. 提取整图CLIP特征
        2. 使用检测器检测物体
        ...
        # 提取整图特征
        image_feature = self.clip_model.encode_image(image, normalize=True)
        ...
        try:
            ...
            for det in detections:
                ...
                if obj_image.size > 0 and obj_image.shape[0] > 5 and obj_image.shape[1] > 5:
                    # 提取物体特征
                    obj_feature = self.clip_model.encode_image(obj_image, normalize=True)
    ...
    def process_images_batch(self, images: List[np.ndarray]) -> List[FrameData]:
        """
        批量处理图像列表
        ...
        for i in range(0, len(images), self.batch_size):
            ...
            for j, image in enumerate(batch_images):
                frame_idx = i + j
                try:
                    frame_data = self.process_single_image(image, frame_idx)
    ...
    def select_keyframes_for_nouns(self, nouns: List[NounInfo], similarity_matrix: np.ndarray,
        ...
        if not unique_sorted_indices and len(frames_data) > 0:
            ...
            top_indices = np.argsort(frame_scores)[-3:][::-1]
    ...
    def process_images(self, images: List[np.ndarray], instruction_text: str) -> Dict[str, Any]:
        """
        完整的处理流程 - 直接处理图像列表
