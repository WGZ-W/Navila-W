...
from llava.model.history_transformer.builder import build_history_transformer
...
class LlavaMetaModel(ABC):
    def init_vlm(self, config: PreTrainedModel = None, *args, **kwargs):
        ...
        self.mm_projector = build_mm_projector(mm_projector_cfg, config)
        # transformer setting
        # history_transformer_path = None
        # if config.resume_path and os.path.exists(os.path.join(config.resume_path, 'history_transformer')):
        #     candidate = os.path.join(config.resume_path, 'history_transformer', 'history_transformer.pth')
        #     if os.path.exists(candidate):
        #         history_transformer_path = candidate
        # self.history_transformer = build_history_transformer(config, history_transformer_path)
    ...
    def save_pretrained(self, output_dir, state_dict=None):
        ...
        if self.get_history_transformer() is not None:
            print(f"saving history_transformer to {osp.join(output_dir, 'history_transformer')}")
            history_transformer_state_dict = OrderedDict(
                {k.split("history_transformer.")[-1]: v for k, v in state_dict.items() if "history_transformer" in k}
            )
            save_dir = osp.join(output_dir, "history_transformer")
            os.makedirs(save_dir, exist_ok=True)
            torch.save(history_transformer_state_dict, osp.join(save_dir, "history_transformer.pth"))
    ...
    def get_history_transformer(self):
        history_transformer = getattr(self, "history_transformer", None)
        if type(history_transformer) is list:
            history_transformer = history_transformer[0]
        return history_transformer
