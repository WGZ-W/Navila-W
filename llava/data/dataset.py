    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        for instance in instances:
            ...
            if instance.get("image") is not None:
                ...
                if not isinstance(instance["input_ids"], list):
                    # dataset other than coyo, not packing >1 samples together
...
# 构建训练数据集和数据收集器（data collator），支持数据混合和序列并行等高级特性。
def make_supervised_data_module(
    ...
    # training_args.eval_sample_lens = [len(d) for d in eval_dataset.dataset]
